...
-- Try to find `subliminal` in $PATH
local handle = io.popen("command -v subliminal 2>/dev/null")
if handle then
    subliminal = handle:read("*l")
    handle:close()
end

-- Fallback to common pipx/local paths if not found in PATH
if not subliminal or subliminal == "" then
    local home = os.getenv("HOME")
    local candidates = {
        home and home .. "/.local/bin/subliminal" or nil,
        "/usr/bin/subliminal",
...
if not subliminal then
    mp.msg.error("subliminal not found in PATH. Install it or set the path manually.")
    return
end
--=============================================================================
-->>    SUBTITLE LANGUAGE:
